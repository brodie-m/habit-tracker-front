...
const baseUrl = "https://fp-habitab.herokuapp.com";
let inc = 0;
...
async function getHabits() {
  ...
  const result = await fetch(`${baseUrl}/api/habits/show`, options);
...
async function getHabitById(id) {
  ...
  const result = await fetch(`${baseUrl}/api/habits/show/${id}`, options);
...
async function drawHabits(result) {
  ...
  data.forEach((habit) => {
    ...
    topTask.style.background = `linear-gradient(90deg, rgba(${
      255 - completionFrac * 255
    },50,${completionFrac * 255},0.3) ${completionFrac * 100 - 5}%, rgba(${
      255 - completionFrac * 255
    },50,${completionFrac * 255},0.3)  ${
      completionFrac * 100
    }%, rgba(244,244,246,1) ${
...
async function submitHabitHandler(event) {
  ...
  const result = await fetch(
    "https://fp-habitab.herokuapp.com/api/habits/add",
    options
  );
...
deleteHabitButton.addEventListener("click", async (e) => {
  ...
  await fetch(`${baseUrl}/api/habits/delete/${habitNameUnderscores}`, options);
  window.location.href = "./dashboard.html";
});
...
async function plusHandler(e) {
  ...
  taskBox.style = `background: linear-gradient(90deg, rgba(${
    255 - newFrac * 255
  },50,${newFrac * 255},0.3) ${newFrac * 100 - 5}%, rgba(${
    255 - newFrac * 255
  },50,${newFrac * 255},0.3) ${newFrac * 100}%, rgba(244,244,246,1) ${
    newFrac * 100 + 1
  }%, rgba(244,244,246,1) 100% )`;
  ...
  const result = await fetch(
    `${baseUrl}/api/habits/updatecurrent/${index}`,
    options
  );
}
...
async function EditFormHandler(event) {
  ...
  const targetHabitData = await fetch(`${baseUrl}/api/habits/show/${id}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      "auth-token": token,
